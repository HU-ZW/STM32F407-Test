...
//LCD的画笔颜色和背景色	   
u16 POINT_COLOR=0xFFFF;	//画笔颜色  白
u16 BACK_COLOR=0x0000;  //背景色   黑
...
void Picture_Draw(u16 S_x,u16 S_y,const unsigned char *pic)
{
        ...
        while(i<(len+8))      
        {
                temp=(pic[i]+(pic[i+1]<<8));     
                if(D_x > x) 
				{
					D_x=0;
					D_y++;
				}         
                LCD_Fast_DrawPoint(S_x+D_x,S_y+D_y,temp);    
                D_x++;
                i=i+2;
        }
}
...
//显示汉字 
//工具: PCtoLCD2002完美版 设置->选项 取模方式逐列式 取模走向顺向
//x,y:起点坐标
//size:字体大小 大小为8的整数倍
//len:字数
//*p:汉字数组地址
//eg: CH_Draw(0, 0, 16, 3, jiayou); 
void CH_Draw(u16 x, u16 y, u8 size,u8 len, const unsigned char *p)
{
	int i=0, j=0;
	unsigned char temp=0x01;
	u16 temp_x=x, temp_y=y;
	for( i=0;i<(size*size/8)*len;i++ )	//size*size/8 汉字对应得点数
		{
			for( j=7;j>=0;j-- )
			{
				//if( (p[i]&(temp<<j)) != 0 )	LCD_DrawPoint(x,y);//求每个点的亮灭
				if( (p[i]&(temp<<j)) != 0 )	//求每个点的亮灭
					LCD_Fast_DrawPoint( x, y, POINT_COLOR);
				else
					LCD_Fast_DrawPoint( x, y, BACK_COLOR);
				
				y++;
				if( y>=temp_y+size )//一行满后,下一行
				{
					y=temp_y;//回到起点
					x++;
					if( x>=temp_x+size)//这个字写完
					{
						temp_x += size;//x起点后移
						y = temp_y;//y回到起点 
						x = temp_x;//x获取新的起点
			...
			if( temp_x+size >= lcddev.width )//x超出边缘
			{
				x = 0;
				temp_x = 0;
				temp_y += size;//y起点下移
			...
			if( temp_y+size >= lcddev.height )//y超出边缘
			{
				temp_x = 0;
				temp_y = 0;
				x = 0;
...
//滚动显示汉字
//x,y:起点坐标
//width, height:字体大小 大小为8的整数倍
//len:字数
//v:滚动速度 10为最快 字体越大速度越慢
//*p:汉字数组地址
//eg: Roll_LCD(0, 100, 16, 32, 7, Roll_Medium_Speed, zi);16x32字体
void Roll_LCD(u16 x, u16 y, u8 width, u8 height, u8 len, Speed_ENU Speed, const unsigned char *p)  
{
	int i=0, j=0;
	unsigned char temp=0x01;
	int temp_x=x, temp_y=y, Temp=x;//起点保存
	static int Dis = 999;//随意给的一个不可能得到的数
	if( Dis == 999 )
		Dis = x;
	
	for( i=0; i<(width*height/8)*len; i++ )	//size*size/8 汉字对应得点数
	{
		
			for( j=7; j>=0; j-- )
			{
				if( 2*x-Dis>=0 && 2*x-Dis<=lcddev.width )
				{
					if( (p[i]&(temp<<j)) != 0 )	//求每个点的亮灭
						LCD_Fast_DrawPoint( 2*x-Dis, y, POINT_COLOR);
					else
						LCD_Fast_DrawPoint( 2*x-Dis, y, BACK_COLOR);
				}
					
				y++;
				if( y>=temp_y+height )//一行满后,下一行
				{
					y=temp_y;//回到起点
					x++;
					if( x>=temp_x+width )//这个字写完
					{
						temp_x += width;//x起点后移
						x = temp_x;//x获取新的起点
						y = temp_y;//y回到起点 
	...
	delay_ms(Speed);//延时
	Dis++;
	if( Dis > (width*len)*2+(Temp*2) )	Dis=Temp-lcddev.width;//显示完后 将点移到最后面
}
